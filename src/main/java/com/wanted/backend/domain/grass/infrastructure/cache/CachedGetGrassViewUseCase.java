...
public class CachedGetGrassViewUseCase implements GetGrassViewUseCase {
    ...
    @Override
    @Cacheable(cacheNames = "grassView:v3", key = "#query.memberId() + ':' + (#query.view() == null ? null : #query.view().trim().toLowerCase()) + ':' + #query.year() + ':' + #query.month() + ':' + T(java.time.LocalDate).now(@clock)")
